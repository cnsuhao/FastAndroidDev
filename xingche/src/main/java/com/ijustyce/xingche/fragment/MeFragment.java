...
import com.ijustyce.xingche.activity.AboutActivity;
import com.ijustyce.xingche.constant.Constant;
import com.ijustyce.xingche.constant.HttpConstant;
import com.ijustyce.xingche.http.HttpAccount;
import com.ijustyce.xingche.model.UserInfo;
...
//                getActivity().finish();
    ...
    public void onSuccess(String object, String taskId){
        ...
        if (!HttpConstant.UPLOAD.equals(taskId)){
            ...
            ImageUtils.load(mContext, header,
                    HttpConstant.FILES + AppApplication.getHead());
            return;
        }else{
        ...
        if (result != null){
            ...
            if (data != null) {
                data.setHead(head);
                tmp.setData(data);
                AppApplication.setUserInfo(tmp);
    ...
    public void doResume() {
        ...
        userName.setText(isLogin ? AppApplication.getUserInfo().getName() : "");
        userMobile.setText(isLogin ? AppApplication.getUserInfo().getPhone() : "");

        String file = FileUtils.getAvailablePath(mContext, Constant.FILE_PATH);
        cacheSize.setText(file == null ? "0M" : "" + FileUtils.getDirSize(new File(file)));

        if (AppApplication.getHead() == null){
            return;
        }
        ImageUtils.load(mContext, header,
                HttpConstant.FILES + AppApplication.getHead());
