...
public class RegisterActivity extends BaseActivity<UserInfo> {
    ...
    public void onSuccess(String object, String taskId) {
        ...
        IApplication.saveUserInfo(userInfo);
    ...
    private synchronized void register(){
        ...
        showProcess("正在注册");
        if(!HttpUser.newUser(getTAG(), phoneText, passwordText, nameText, identity, mContext, httpListener)){
            dismiss();
        }
    }
    ...
    @OnClick({R.id.back, R.id.register, R.id.isStudent, R.id.isTeacher})
